...
export const CurriculumLibraryView: React.FC<CurriculumLibraryViewProps> = ({
  ...
  useEffect(() => {
    ...
    const id = setTimeout(() => {
      ...
      if (contentEl) {
        ...
        window.scrollTo({
          top: Math.max(0, 200 - targetFromTop),
          behavior: "smooth",
        });
      }
    }, 80);
  ...
  }, [data.sections, searchQuery, activeAxis, activeYear, stage]);
  ...
  const DetailedSection = ({
    ...
    const getAxisColors = (axisTitle: string) => {
      ...
      if (t.includes("design") || t.includes("fabricação"))
        return {
          bg: "bg-[#3D1D3D]",
          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-8 lg:mb-12 relative z-10">
            <div className="flex items-center gap-4 lg:gap-6">
              {hasIcon && (
                <div
                  className={cn(
                    "p-3 lg:p-4 rounded-xl lg:rounded-2xl shadow-sm border border-zinc-100 bg-white",
                    colors.lightText,
                  )}
                >
                  {React.cloneElement(
                    getIconByTitle(title) as React.ReactElement,
              ...
              <div className="space-y-1">
                <span
                  className={cn(
                    "text-[9px] lg:text-[10px] font-black uppercase tracking-[2px] lg:tracking-[3px] opacity-70",
                    colors.lightText,
                  )}
                >
                  Eixos Estruturantes
                ...
                <h2
                  className={cn(
                    "text-[24px] lg:text-[40px] font-black tracking-tight leading-tight text-[#1B2C49]",
                  )}
                >
                  {title}
                </h2>
          ...
          <div className="space-y-6 lg:space-y-8 relative z-10">
            {items.map((item, idx) => {
              ...
              lines.forEach((line: string) => {
                ...
                const isObjeto =
                  trimmed.toLowerCase().startsWith("objeto de conhecimento") ||
                  trimmed.toLowerCase().startsWith("objetos de conhecimento");
                const isCompetencia =
                  trimmed.toLowerCase().startsWith("competência") ||
                  trimmed.toLowerCase().startsWith("competências");
                const isHabilidade =
                  trimmed.startsWith("(") ||
              ...
              return (
                <Collapsible
                  ...
                  defaultOpen={idx === 0}
                  className={cn(
                    "relative rounded-[24px] lg:rounded-[32px] border transition-all overflow-hidden",
                    isGeneral
                  ...
                  <div className="p-5 lg:p-8">
                    <div className="flex flex-col gap-6 lg:gap-8">
                      <div className="flex items-center justify-between gap-4">
                        <CollapsibleTrigger className="flex items-center gap-4 lg:gap-5 flex-1 text-left cursor-pointer group/trigger hover:opacity-90 transition-opacity">
                          {itemIcon && (
                            <div
                              className={cn(
                                "p-3 lg:p-4 rounded-lg lg:rounded-xl shadow-sm ring-1 ring-zinc-100 bg-white shrink-0",
                                colors.lightText,
                              )}
                            >
                              {itemIcon}
                          ...
                          <h3 className="text-[18px] lg:text-[24px] font-bold text-[#1B2C49] leading-tight">
                            {item.title}
                          </h3>
                          <ChevronDown className="size-5 text-zinc-400 shrink-0 transition-transform duration-200 group-data-[state=open]/trigger:rotate-180" />
                        </CollapsibleTrigger>
                        <CopyButton text={`${item.title}\n${item.description}`} className="shrink-0" />
                      </div>
      ...
      <div className="mt-16 bg-white rounded-[32px] border border-zinc-100 p-10 shadow-[0px_24px_48px_-12px_rgba(27,44,73,0.06)]">
        ...
        <div className="flex flex-wrap gap-3">
          {items.map((item, idx) => (
            <div
              key={idx}
              className="px-5 py-3 rounded-full bg-[#F1EEF9]/50 border border-zinc-100 text-[14px] font-bold text-[#1B2C49]"
            >
              {item}
            </div>
          ))}
  ...
  const GeneralCompetencies = () => {
    const competencies =
      ...
      Array.isArray(data.generalCompetencies) &&
      data.generalCompetencies.length > 0
        ? data.generalCompetencies
        : [
            {
      ...
      <div className="space-y-10 mt-16 md:mt-24 mb-16">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="space-y-2">
            <span className="text-[10px] font-black uppercase tracking-[3px] text-[#E7609F]">
              Nave a Vela + BNCC
            </span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
          {competencies.map((comp, idx) => (
            <div
              key={idx}
              className="bg-white p-6 md:p-10 rounded-[32px] md:rounded-[40px] border border-zinc-100 shadow-[0px_16px_32px_-12px_rgba(27,44,73,0.05)] hover:shadow-xl hover:border-[#E7609F]/20 transition-all group relative"
            >
              <div className="absolute top-6 right-6 md:top-8 md:right-8">
                <CopyButton text={`${comp.title}\n${comp.description}`} />
              </div>
              <h4 className="text-[18px] md:text-[20px] font-bold text-[#1B2C49] mb-3 md:mb-4 group-hover:text-[#E7609F] transition-colors leading-tight pr-12 md:pr-12">
                {comp.title}
              </h4>
              <p className="text-[14px] md:text-[15px] leading-relaxed text-[#71717B] font-light">
                {comp.description}
              </p>
            </div>
          ))}
  ...
  const WEFSkills = () => {
    const wefData =
      data.wefSkills &&
      Array.isArray(data.wefSkills) &&
      data.wefSkills.length > 0
        ? data.wefSkills.map(
            (c: { title: string; subtitle: string; skills: string[] }) => ({
              title: c.title,
              subtitle: c.subtitle,
              skills: c.skills,
              color: "bg-[#4C76BA]",
    ...
    const categories =
      wefData.length > 0
        ? wefData.map((c, i) => ({
            ...c,
            color: colorMap[i] || "bg-[#4C76BA]",
            dotColor: colorMap[i] || "bg-[#4C76BA]",
          }))
        : [
            {
              title: "Competências",
    ...
    return (
      <div className="space-y-8 lg:space-y-12 mt-20 lg:mt-32 mb-16">
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
          <div className="space-y-2">
            <span className="text-[9px] lg:text-[10px] font-black uppercase tracking-[2px] lg:tracking-[3px] text-[#94579E]">
              World Economic Forum
            </span>
            <h2 className="text-[28px] lg:text-[32px] font-black text-[#1B2C49] tracking-tight leading-tight">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
          {categories.map((cat, idx) => (
            <div
              key={idx}
              className="flex flex-col h-full bg-white rounded-[40px] lg:rounded-[60px] border border-zinc-100 overflow-hidden shadow-[0px_20px_40px_-12px_rgba(27,44,73,0.08)] transition-all group"
            >
              <div
                className={cn(
                  "p-8 lg:p-12 pb-6 lg:pb-8 flex flex-col relative",
                  cat.color,
                )}
              >
                <div className="flex justify-between items-start gap-4">
                  <h4 className="text-[20px] lg:text-[26px] font-black text-white leading-[1.1] max-w-[200px] min-h-[2.2em]">
                    {cat.title}
                  </h4>
                  <CopyButton
                ...
                <p className="text-[13px] lg:text-[15px] text-white/90 font-medium leading-snug mt-5 lg:mt-6 min-h-[2.6em]">
                  {cat.subtitle}
                </p>
              </div>
              <div className="p-8 lg:p-12 pt-6 lg:pt-10 flex-1 space-y-4 lg:space-y-5 bg-white">
                {cat.skills.map((skill, sIdx) => (
                  <div
                    key={sIdx}
                    className="flex items-center gap-3 lg:gap-4 group/skill"
                  >
                    <div
                      className={cn(
                        "size-1.5 lg:size-2 rounded-full shrink-0",
                        cat.dotColor,
                      )}
                    />
                    <span className="text-[14px] lg:text-[16px] font-bold text-[#1B2C49] leading-tight transition-colors">
                      {skill}
                    </span>
                  </div>
                ))}
      ...
      <main className="relative z-10 w-full p-4 md:p-8 lg:p-16 scroll-smooth">
        <div className="mx-auto max-w-[1300px] flex flex-col lg:flex-row gap-6 md:gap-8 items-start">
          ...
          <div className="flex-1 min-w-0 w-full max-w-[1100px] mx-auto">
                    ...
                    <div className="space-y-6 lg:space-y-8 text-[#6B6B6B] text-[14px] lg:text-[15px] leading-[1.6]">
                      <div className="space-y-4 lg:space-y-3">
                        <p className="flex flex-col lg:flex-row lg:items-baseline gap-2 lg:gap-3">
                          <span className="font-black text-[#1B2C49] uppercase text-[9px] lg:text-[11px] tracking-widest bg-zinc-50 px-2.5 py-1 rounded-md w-fit">
                            Foco
                          </span>
                          <span className="text-[16px] lg:text-[18px] font-medium text-[#1B2C49]">
                            {data.hero.foco}
                          </span>
                        </p>
                        <p className="flex flex-col lg:flex-row lg:items-baseline gap-2 lg:gap-3">
                          ...
                          <span className="text-[16px] lg:text-[18px] font-medium text-[#1B2C49]">
                            {data.hero.valor_central}
                          </span>
                        </p>
                      </div>
                      <div className="space-y-4 lg:space-y-6 font-light max-w-[860px] text-[#71717B] text-[16px]">
                        {data.hero.description_blocks &&
                          data.hero.description_blocks.map(
            ...
            <div
              ref={filtersBlockRef}
              className="sticky top-0 z-20 pb-4 lg:pb-8 mb-4"
            >
              ...
              <div className="relative pt-2 space-y-4 lg:space-y-6">
                ...
                <div className="space-y-4 lg:space-y-6">
                  {/* Search Bar - oculto quando "Todos" em ano pois não há conteúdo para pesquisar */}
                  {(!(stage === "ef1" || stage === "ef2" || stage === "em") ||
                    activeYear !== "Todos") && (
            ...
            <div ref={contentBlockRef} className="pb-20 lg:pb-32">
              {(stage === "ef1" ||
                ...
                <motion.div
                  ...
                  className="mt-12 lg:mt-16 flex flex-col items-center text-center space-y-6"
                >
                ...
                >
                  ...
                  <div className="space-y-2 max-w-[400px]">
                    <h3 className="text-[20px] font-bold text-[#1B2C49]">
                      Nenhum conteúdo encontrado
                    </h3>
                    <p className="text-[15px] text-zinc-500 font-light leading-relaxed">
                      Não encontramos resultados para os filtros selecionados ou
                      para o termo pesquisado. Tente ajustar sua busca.
                    </p>
                  </div>
                  <button
                    onClick={() => {
